...
class Cloth(base.Task):
  ...
  def action_spec(self, physics):
    ...
    return specs.BoundedArray(
          shape=(3,), dtype=np.double, minimum=[-1.0] * 3, maximum=[1.0] * 3)
  ...
  def get_reward(self, physics):
    ...
    array = ([-0.09,-0.15,0.],[-0.09,-0.12,0.],[-0.09,-0.09,0.],[-0.09,-0.06,0.])
    print(array[0])
    print(physics.named.data.geom_xpos['G0_0'])
    if np.all(physics.named.data.geom_xpos['G0_0']==array[0]):
        reward = +1
    if np.all(physics.named.data.geom_xpos['G0_1']==array[1]):
        reward = +1
    if np.all(physics.named.data.geom_xpos['G0_2']==array[2]):
        reward = +1
    if np.all(physics.named.data.geom_xpos['G0_3']==array[3]):
        reward = +1
    else: 
        reward = -2
    return reward
