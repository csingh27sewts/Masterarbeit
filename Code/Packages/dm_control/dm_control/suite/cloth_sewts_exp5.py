...
class Cloth(base.Task):
  ...
  def action_spec(self, physics):
    ...
    # LEFT CORNER + ITS NEIGHBOURS ACTION [x,y,z] 3*4 = 12
  ...
  def get_observation(self, physics):
    ...
    obs.append(physics.named.data.geom_xpos['G0_1'].reshape(-1).astype('float32'))
    obs.append(physics.named.data.geom_xpos['G0_2'].reshape(-1).astype('float32'))
    obs.append(physics.named.data.geom_xpos['G0_3'].reshape(-1).astype('float32'))
    print(obs)
    return obs
